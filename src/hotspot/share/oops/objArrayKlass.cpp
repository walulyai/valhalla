...
Symbol* ObjArrayKlass::create_element_klass_array_name(Klass* element_klass, TRAPS) {
  ResourceMark rm(THREAD);
  char *name_str = element_klass->name()->as_C_string();
  int len = element_klass->name()->utf8_length();
  char* new_str = NEW_RESOURCE_ARRAY_IN_THREAD(THREAD, char, len + 4);
  int idx = 0;
...
ObjArrayKlass* ObjArrayKlass::allocate_objArray_klass(ClassLoaderData* loader_data,
  ...
  Symbol* name = create_element_klass_array_name(element_klass, CHECK_NULL);
